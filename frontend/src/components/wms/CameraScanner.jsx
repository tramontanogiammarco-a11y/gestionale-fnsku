...
export default function CameraScanner({ open, onOpenChange, purpose = "universal", onDetected, context = null }) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className={`wms-shell max-w-[calc(100%-8px)] rounded-md border-slate-300 bg-[#f8faf9] sm:max-w-md ${compactPicking ? "flex h-[calc(100dvh-8px)] max-h-[calc(100dvh-8px)] flex-col gap-2 overflow-hidden p-2 sm:p-3" : "block max-h-[calc(100dvh-16px)] space-y-3 overflow-y-auto p-3 sm:p-4"}`}>
        <DialogHeader className={compactPicking ? "shrink-0 space-y-0 pr-8" : ""}>
          <DialogTitle className={compactPicking ? "text-lg leading-tight" : ""}>{title}</DialogTitle>
          <DialogDescription className={compactPicking ? "truncate text-xs leading-4" : ""}>{description}</DialogDescription>
        </DialogHeader>
        {context && (compactPicking ? <section className="shrink-0 overflow-hidden rounded-md border border-slate-200 bg-white">
          <div className="flex items-center gap-2 p-2">
            <div className="min-w-0 flex-1">
              ...
              <div className="mt-0.5 flex items-end gap-2">
                ...
                <span className="mb-0.5 shrink-0 rounded-md bg-teal-700 px-2 py-1 text-xs font-black text-white">{context.requested} pz</span>
              </div>
          ...
          {context.bagAllocations?.length > 0 && <div className="border-t border-amber-200 bg-amber-50/60 px-2 py-1.5">
            ...
            <div className="mt-1 flex gap-1.5 overflow-x-auto pb-0.5">
              {context.bagAllocations.map((allocation) => <div key={`${allocation.posizione_bag}-${allocation.bag_code}`} className="flex min-w-[116px] shrink-0 items-center justify-between rounded-md border border-amber-300 bg-amber-50 px-2 py-1.5">
                <span><span className="block text-[9px] font-black uppercase leading-none text-amber-800">Bag {allocation.posizione_bag}</span><strong className="mt-0.5 block font-mono text-xs leading-none text-slate-950">{allocation.bag_code}</strong></span>
                <strong className="text-lg font-black leading-none text-amber-950">x{allocation.quantita}</strong>
              </div>)}
            </div>
          </div>}
        </section> : <section className="rounded-md border border-slate-200 bg-white">
        ...
        <div className={`relative overflow-hidden rounded-md bg-slate-950 shadow-inner ${compactPicking ? "min-h-0 flex-1" : "aspect-[4/5] max-h-[64dvh]"}`}>
          ...
          {context?.imageUrl && <figure className={`pointer-events-none absolute left-1/2 z-20 -translate-x-1/2 overflow-hidden rounded-md border-2 border-white bg-white shadow-lg ${compactPicking ? "top-2 w-[90%] p-1.5" : "top-[4%] w-[88%] p-2"}`}>
            <img src={context.imageUrl} alt={context.title || "Prodotto da prelevare"} className={compactPicking ? "h-[clamp(120px,25dvh,190px)] w-full object-contain" : "h-52 w-full object-contain"} decoding="async" fetchPriority="high" />
