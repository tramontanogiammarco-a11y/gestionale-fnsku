...
function isVerifiedQueuedOrder(order = {}) {
...
function isWmsOperationalOrder(order = {}) {
  return ["da_preparare", "in_preparazione"].includes(order.wms_status)
    && order.gate_status === "sbloccato";
}

async function reservationsForQueuedOrderIds(orderIds = []) {
...
async function queuedSlotReservationsBefore(order) {
  const { data: queuedOrders, error: ordersError } = await requireSupabase()
    .from("shopify_orders")
    .select("id,processed_at,created_at,wms_status,gate_status")
    .eq("cliente_id", order.cliente_id)
    .in("wms_status", ["da_preparare", "in_attesa_refill"]);
  if (ordersError) fail(ordersError.message);

  const verifiedOrders = (queuedOrders || []).filter(isVerifiedQueuedOrder);
  const ordered = [...verifiedOrders.filter((row) => row.id !== order.id), order]
    .sort((left, right) => {
  ...
  return reservationsForQueuedOrderIds(priorIds);
}

async function verifiedQueuedReservationsExcept(clienteId, excludedOrderIds = [], options = {}) {
  const excluded = new Set(excludedOrderIds.filter(Boolean));
  const { data: queuedOrders, error } = await requireSupabase()
    .from("shopify_orders")
    .select("id,processed_at,created_at,wms_status,gate_status")
    .eq("cliente_id", clienteId)
    .in("wms_status", ["da_preparare", "in_attesa_refill"]);
  ...
  const beforeDate = String(before?.processed_at || before?.created_at || "");
  const reservingIds = (queuedOrders || [])
    .filter(isVerifiedQueuedOrder)
    .filter((order) => !excluded.has(order.id))
    .filter((order) => {
      if (!before) return true;
      const orderDate = String(order.processed_at || order.created_at || "");
      return orderDate < beforeDate || (orderDate === beforeDate && String(order.id) < String(before.id));
    })
    .map((order) => order.id);
...
async function evaluateWmsOrderGateLegacy(orderId, options = {}) {
  const profile = await assertWmsStaff();
  const { data: order, error: orderError } = await requireSupabase()
    .from("shopify_orders")
    .select("*")
...
async function evaluateWmsOrderGate(orderId, options = {}) {
  const { data: order, error } = await requireSupabase()
    ...
    .eq("id", orderId)
    .single();
  if (error || !order) fail(error?.message || "Ordine non trovato", 404);
  const lockedStatuses = ["in_preparazione", "in_attesa_packing", "in_packing", "imballato", "spedito", "annullato"];
  if (lockedStatuses.includes(order.wms_status) && !options.force) return { ...order, skipped: true };

  const response = await recheckWmsOrderExceptions({
    order_id: orderId,
    include_ready: Boolean(options.force),
    limit: 1,
  });
  const result = response.data?.results?.find((row) => row.id === orderId);
  if (result?.error) fail(result.error);
  if (!result?.order) fail("L'ordine non e in uno stato ricontrollabile", 409);
  return result.order;
}
...
async function reservedWmsPickingQuantity(locationId, productKey, clienteId) {
  const [
  ...
  const taskOrderIds = [...new Set((tasks || []).map((row) => row.order_id).filter(Boolean))];
  const { data: taskOrders, error: taskOrdersError } = taskOrderIds.length
    ? await requireSupabase().from("shopify_orders").select("id,wms_status,gate_status").eq("cliente_id", clienteId).in("id", taskOrderIds)
    : { data: [], error: null };
  if (taskOrdersError) fail(taskOrdersError.message);
  const reservingOrderIds = new Set((taskOrders || [])
    .filter((row) => row.gate_status === "sbloccato" && ["da_preparare", "in_preparazione"].includes(row.wms_status))
    .map((row) => row.id));
  const taskIds = (tasks || []).filter((row) => reservingOrderIds.has(row.order_id)).map((row) => row.id);
...
async function moveWmsStockQuantity(payload = {}) {
  ...
  const reserved = await reservedWmsPickingQuantity(source.id, item.product_key, item.cliente_id);
...
const wmsReferencePhotoCache = new Map();
const WMS_REFERENCE_PHOTO_CACHE_MS = 5 * 60 * 1000;
...
async function withWmsReferencePhotos(lines = [], clienteId = null) {
  ...
  const cacheKey = `${clienteId || "all"}:${lookups.map(({ field, values }) => `${field}=${[...values].sort().join(",")}`).join("|")}`;
  const cached = wmsReferencePhotoCache.get(cacheKey);
  let results;
  if (cached && Date.now() - cached.createdAt < WMS_REFERENCE_PHOTO_CACHE_MS) {
    results = cached.results;
  } else {
    results = await Promise.all(lookups.map(async ({ field, values }) => {
    ...
    if (wmsReferencePhotoCache.size >= 200) wmsReferencePhotoCache.clear();
    wmsReferencePhotoCache.set(cacheKey, { createdAt: Date.now(), results });
  }
...
function monoCandidateOrders(orders = [], excludedOrderIds = new Set()) {
  return (orders || []).filter((order) => (
    ...
    && order.raw?.preparation_method !== "galluse"
...
async function wmsBagContents(rawCode) {
  await assertWmsStaff();
  const code = normalizedScanCode(rawCode);
  if (!/^B-[A-Z0-9]{5}$/.test(code)) fail("Codice bag non valido");
  const sb = requireSupabase();
  const { data: bag, error: bagError } = await sb.from("wms_bags").select("id,codice,stato,updated_at").eq("codice", code).maybeSingle();
  if (bagError || !bag) fail(bagError?.message || "Bag non trovata", 404);

  const { data: sessions, error: sessionsError } = await sb
    .from("wms_packing_sessions")
    .select("id,order_id,stato,created_at")
    .eq("bag_code", code)
    .neq("stato", "annullata")
    .neq("stato", "completata")
    .order("created_at", { ascending: false });
  if (sessionsError) fail(sessionsError.message);
  let orderIds = [...new Set((sessions || []).map((session) => session.order_id).filter(Boolean))];
  let phase = orderIds.length ? "packing" : null;

  if (!orderIds.length && bag.stato !== "disponibile") {
    const { data: links, error: linksError } = await sb
      .from("wms_galluse_orders")
      .select("batch_id,order_id,created_at")
      .eq("bag_code", code)
      .order("created_at", { ascending: false })
      .limit(20);
    if (linksError) fail(linksError.message);
    const batchIds = [...new Set((links || []).map((link) => link.batch_id).filter(Boolean))];
    const { data: batches, error: batchesError } = batchIds.length
      ? await sb.from("wms_galluse_batches").select("id,stato").in("id", batchIds).in("stato", ["in_corso", "completata"])
      : { data: [], error: null };
    if (batchesError) fail(batchesError.message);
    const activeBatchIds = new Set((batches || []).map((batch) => batch.id));
    orderIds = [...new Set((links || []).filter((link) => activeBatchIds.has(link.batch_id)).map((link) => link.order_id).filter(Boolean))];
    if (orderIds.length) phase = "picking_galluse";
  }

  const { data: orders, error: ordersError } = orderIds.length
    ? await sb.from("shopify_orders").select("*").in("id", orderIds)
    : { data: [], error: null };
  if (ordersError) fail(ordersError.message);
  const enriched = await enrichShopifyOrders(orders || []);
  const contents = enriched.map((order) => ({
    id: order.id,
    order_name: order.order_name,
    cliente: order.cliente_ragione_sociale,
  ...
  return ok({
    ...
    summary: {
      ...
      references: contents.reduce((sum, order) => sum + order.items.length, 0),
      pieces: contents.reduce((sum, order) => sum + order.items.reduce((itemSum, item) => itemSum + item.quantita, 0), 0),
    },
  });
}
...
async function startWmsMassPicking(payload = {}) {
  ...
  const requestedOrderCount = Math.floor(Number(payload.numero_ordini ?? group.numero_ordini));
  if (!Number.isFinite(requestedOrderCount) || requestedOrderCount < 2 || requestedOrderCount > group.numero_ordini) {
    fail(`Seleziona da 2 a ${group.numero_ordini} ordini identici.`, 422);
  }
  const selectedOrders = [...group.orders]
    .sort((left, right) => (
      String(left.processed_at || left.created_at || "").localeCompare(String(right.processed_at || right.created_at || ""))
      || String(left.id).localeCompare(String(right.id))
    ))
  ...
  const lines = plan.allocations.sort((a, b) => sequenceMap[a.location_id] - sequenceMap[b.location_id]).map((allocation, index) => {
    ...
    return {
      ...
      numero_ordini: selectedOrders.length,
...
async function scanWmsMassPicking(batchId, payload = {}) {
  ...
  const { error: movementError } = await requireSupabase().rpc("record_wms_mass_pick_quantity", {
    p_line_id: current.id,
    p_quantity: quantity,
  });
...
async function reclassifyUnavailableGalluseOrders(candidates = []) {
  ...
  for (const [clientId, clientOrders] of Object.entries(byClient)) {
    ...
    const queuedSlotReserved = await verifiedQueuedReservationsExcept(clientId, orderIds, { before: clientOrders[0] });
...
async function classifyUnavailableOrderFromPlan(order, plan, itemOrderMap) {
  ...
  const orderReplenishment = (plan.replenishment || []).filter((row) => (
    itemOrderMap.get(row.order_item_id) === order.id
  ));
  const refillRequirements = orderReplenishment.filter((row) => (
    Number(row.pallet_available || 0) >= Number(row.quantita || 0) && row.target_slot
  ));
  const stockShortages = orderReplenishment
    .filter((row) => Number(row.pallet_available || 0) < Number(row.quantita || 0) || !row.target_slot)
    .map((row) => ({
      referenza_id: row.referenza_id,
      titolo: row.titolo,
  ...
  const hasUnmappedError = (plan.unavailableOrderItemIds || []).some((itemId) => (
    itemOrderMap.get(itemId) === order.id
    && !orderReplenishment.some((row) => row.order_item_id === itemId)
  ));
  if (hasUnmappedError) {
    stockShortages.push({
      titolo: "Stock non disponibile o referenza non configurata",
      required: null,
      available: null,
      missing: null,
      reason: "Impossibile creare il piano di picking",
    });
  }
  const waitsForRefill = stockShortages.length === 0 && refillRequirements.length > 0;
  const update = waitsForRefill ? {
    wms_status: "in_attesa_refill",
    gate_status: "attesa_refill",
    exception_type: null,
    exception_reasons: [],
    address_validation: { ...(order.address_validation || {}), requires_replenishment: true },
    stock_shortages: [],
    refill_requirements: refillRequirements,
    gate_checked_at: checkedAt,
    unblocked_at: null,
  ...
  const { error: updateError } = await requireSupabase().from("shopify_orders").update(update).eq("id", order.id);
  if (updateError) fail(updateError.message);
  const { error: eventError } = await requireSupabase().from("wms_order_gate_events").insert({
    order_id: order.id,
    cliente_id: order.cliente_id,
    from_status: order.gate_status || "sbloccato",
    to_status: update.gate_status,
    reason: waitsForRefill ? "Stock disponibile a pallet: rifornimento slot richiesto" : "Stock insufficiente",
    details: { stock_shortages: update.stock_shortages, refill_requirements: update.refill_requirements },
    created_by: profile.id,
  });
  if (eventError) fail(eventError.message);
}
...
async function startWmsGallusePicking(payload = {}) {
  ...
  let orders = candidates.filter((order) => order.cliente_id === clientId).slice(0, capacity);
  if (!clientId || orders.length < 1) fail("Non ci sono ordini disponibili per il carrello Galluse.", 409);
  let combinedItems = orders.flatMap((order) => (order.items || []).map((item) => ({ ...item, galluse_order_id: order.id })));
  let itemOrderMap = new Map(combinedItems.map((item) => [item.id, item.galluse_order_id]));
  const queuedSlotReserved = await verifiedQueuedReservationsExcept(clientId, orders.map((order) => order.id), { before: orders[0] });
  let plan = await wmsPickingPlan({ cliente_id: clientId }, combinedItems, { queuedSlotReserved });
  ...
  if (unavailableOrderIds.size) {
    ...
    combinedItems = orders.flatMap((order) => (order.items || []).map((item) => ({ ...item, galluse_order_id: order.id })));
    itemOrderMap = new Map(combinedItems.map((item) => [item.id, item.galluse_order_id]));
    plan = await wmsPickingPlan({ cliente_id: clientId }, combinedItems, { queuedSlotReserved });
  }
  if (plan.errors.length) fail(plan.errors.join(" "));
  if (plan.replenishment.length) fail(`Rifornisci prima gli slot per ${plan.replenishment.length} ${plan.replenishment.length === 1 ? "prodotto" : "prodotti"}.`);
  const uniqueLocations = [...new Set(plan.allocations.map((allocation) => allocation.location_id))].map((id) => plan.locationMap[id]).filter(Boolean);
  ...
  const { error: allocationsError } = await requireSupabase().from("wms_galluse_allocations").insert(plan.allocations.map((allocation) => ({
    ...
    galluse_order_id: linkByOrderId[itemOrderMap.get(allocation.order_item_id)].id,
...
async function scanWmsGallusePicking(batchId, payload = {}) {
  ...
  const { error: movementError } = await requireSupabase().rpc("record_wms_galluse_pick_quantity", {
    p_line_id: current.id,
    p_quantity: quantity,
  });
  if (movementError) fail(movementError.message);
...
async function resetWmsHomeStockCatalog() {
  const profile = await assertWmsAdmin();
  let { data: demoClient, error: clientError } = await requireSupabase()
    .from("clienti")
    .select("id,ragione_sociale")
    .eq("ragione_sociale", "WMS Demo Picking")
...
async function startWmsPicking(orderId) {
  ...
  if (order.gate_status !== "sbloccato") fail("Questo ordine non ha ancora superato i controlli automatici", 409);
  ...
  const physicalStops = [...new Map(uniqueLocations.map((location) => [wmsPhysicalBlockKey(location), location])).values()];
  const route = calculateWarehouseRoute(visibleWmsRouteLocations(physicalStops, mapSettings), mapSettings);
  if (route.unreachable?.length) fail(`Mappa bloccata: ${route.unreachable.map((location) => location.codice).join(", ")} non e raggiungibile. Lascia almeno una casella libera di passaggio.`);
  const blockSequence = Object.fromEntries(route.locations.map((location, index) => [wmsPhysicalBlockKey(location), index + 1]));
  const sequenceMap = Object.fromEntries(uniqueLocations.map((location) => [location.id, blockSequence[wmsPhysicalBlockKey(location)] || 9999]));
  allocations.sort((left, right) => (
    sequenceMap[left.location_id] - sequenceMap[right.location_id]
    || naturalLocationSort(locationMap[left.location_id], locationMap[right.location_id])
  ));
...
async function scanWmsPicking(taskId, payload = {}) {
  ...
  if (!current.location_confirmed_at) {
    ...
    if (error) fail(error.message);
    if (!payload.quantita) return wmsPickSnapshot(task.order_id);
  }
  const remaining = Number(current.quantita_attesa || 0) - Number(current.quantita_prelevata || 0);
  const quantity = Math.floor(Number(payload.quantita || remaining));
  if (!Number.isFinite(quantity) || quantity <= 0 || quantity > remaining) fail(`Puoi prelevare da 1 a ${remaining} pezzi.`);
  const { error: movementError } = await requireSupabase().rpc("record_wms_pick_quantity", {
    p_line_id: current.id,
    p_quantity: quantity,
  });
  if (movementError) fail(movementError.message);
...
async function packingStationSnapshot(bagCode) {
  ...
  const { data: packagingOptions, error: packagingError } = hasPendingPackaging
    ? await requireSupabase()
...
async function completePackingLabel(session) {
  const { error } = await requireSupabase().rpc("complete_wms_packing_label", {
    p_session_id: session.id,
    p_label_code: session.carrier_label_code,
  });
  if (error) fail(error.message);
}

async function markWmsPackingLabelsPrinted(payload = {}) {
  await assertWmsStaff();
  const sessionIds = [...new Set((payload.session_ids || []).filter(Boolean))];
  if (!sessionIds.length) fail("Nessuna etichetta da confermare");
  const { data, error } = await requireSupabase().rpc("mark_wms_packing_labels_printed", {
    p_session_ids: sessionIds,
  });
  if (error) fail(error.message);
  return ok({ printed: Number(data || 0) });
}
...
async function scanWmsPackingStation(payload = {}) {
  ...
  if (!activeBagCode) {
    ...
    return ok({
      ...snapshot.data,
      sessions: snapshot.data.sessions.map((session) => eligible.some((item) => item.id === session.id)
        ? { ...session, stato: "in_verifica_bag", bag_first_scanned_at: scannedAt, updated_at: scannedAt }
        : session),
      packaging_options: [],
      phase: "double_check",
    });
...
async function wmsBagPackingSnapshot(bagCode) {
  ...
  if (!batch) {
    ...
    const [{ data: order, error: orderError }, { data: lines, error: linesError }] = await Promise.all([
      requireSupabase().from("shopify_orders").select("*").eq("id", normalSession.order_id).single(),
      requireSupabase().from("wms_packing_lines").select("*").eq("session_id", normalSession.id).order("created_at", { ascending: true }),
    ]);
    if (orderError || linesError) fail((orderError || linesError).message);
    return ok({ batch: null, sessions: [{ ...normalSession, order, lines: lines || [] }], summary: { orders: 1, completed: normalSession.stato === "completata" ? 1 : 0 } });
  }
...
export const api = {
  async get(url, config = {}) {
    ...
    if (path.match(/^\/wms\/bags\/B-[A-Z0-9]{5}\/contenuto$/)) return wmsBagContents(path.split("/")[3]);
  ...
  async post(url, payload, config = {}) {
    ...
    if (path === "/wms/packing/labels/printed") return markWmsPackingLabelsPrinted(payload);
