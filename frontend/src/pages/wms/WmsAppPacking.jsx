...
function PackingQueueV2() {
  ...
  const load = useCallback(async () => {
    try {
      const [packingResponse, bagsResponse] = await Promise.all([api.get("/wms/packing"), api.get("/wms/bags")]);
      setSessions(packingResponse.data || []);
  ...
  const activeBags = bags.filter((bag) => bag.stato === "in_packing").map((bag) => ({
    ...bag,
    orders: sessions.filter((session) => session.bag_id === bag.id && session.stato !== "completata"),
  }));
  ...
  return <div className="space-y-5 pb-24" data-testid="wms-packing-queue">
    ...
    {activeBags.length > 0 && <section><div className="mb-3 flex items-center justify-between"><h2 className="text-xl font-black">Bag attive</h2><span className="rounded-full bg-amber-100 px-2 py-1 text-xs font-black text-amber-900">{activeBags.length}</span></div><div className="space-y-2">{activeBags.map((bag) => <button key={bag.id} type="button" onClick={() => navigate(`/wms-app/packing/bag/${bag.codice}`)} className="flex w-full items-center gap-3 rounded-md border border-amber-200 bg-amber-50 p-4 text-left"><span className="flex h-11 w-14 items-center justify-center rounded-md bg-slate-950 font-mono text-sm font-black text-white">{bag.codice}</span><span className="min-w-0 flex-1"><strong className="block">{bag.orders.length} {bag.orders.length === 1 ? "ordine" : "ordini"} in packing</strong><span className="mt-1 block text-xs text-amber-900">Apri per verificare, chiudere i colli e liberare la bag.</span></span><ChevronRight className="h-5 w-5 text-amber-900" /></button>)}</div></section>}
