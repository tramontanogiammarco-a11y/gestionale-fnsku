...
import { getOrCreatePrintStationCode, printStationChannelName, printStationQrValue } from "@/lib/printStation";
...
export default function WmsAppPacking() {
  ...
  const stationQrValue = useMemo(() => printStationQrValue(printStationCode), [printStationCode]);
      ...
      <aside className="flex w-full shrink-0 flex-col items-center gap-3 lg:items-end">
        <section className="w-full rounded-md border border-teal-300 bg-white p-3 shadow-sm">
          <div className="mx-auto w-fit rounded-md border border-slate-200 bg-white p-2">
            <QrCodeSvg value={stationQrValue} size={196} className="h-[196px] w-[196px]" />
          </div>
          <div className="mt-3 flex items-center justify-center gap-2 text-teal-800"><QrCode className="h-4 w-4" /><p className="text-[10px] font-black uppercase">Scansiona dall'app WMS</p></div>
