...
function QueueGroupCard({ group, mode, working, onStart }) {
  ...
  useEffect(() => {
    setSelected((current) => current > 0 ? Math.min(current, available) : available);
  }, [available]);
  ...
  const displayedProducts = useMemo(() => {
    if (mode !== "mono") {
      return group.products.map((product) => ({
        ...product,
        displayedQuantity: selected * Number(product.quantita_per_ordine || 1),
      }));
    }
    if (selected === 0) {
      return group.products.map((product) => ({
        ...product,
        displayedQuantity: Number(product.quantita_totale || 0),
      }));
    }
    const selectedOrders = (group.orders || []).slice(0, selected);
    return group.products.map((product) => ({
      ...product,
      displayedQuantity: selectedOrders.reduce((total, order) => total + (order.items || [])
        .filter((item) => item.referenza_id === product.referenza_id)
        .reduce((sum, item) => sum + Number(item.quantita || 0), 0), 0),
    })).filter((product) => product.displayedQuantity > 0);
  }, [group.orders, group.products, mode, selected]);
  return <article className="rounded-md border border-slate-200 bg-white p-4 shadow-sm">
    ...
    <div className="mt-4 divide-y divide-slate-100 border-y border-slate-100">{displayedProducts.map((product) => <div key={product.referenza_id} className="flex items-center gap-3 py-3"><span className="min-w-0 flex-1"><strong className="block truncate text-sm">{product.titolo}</strong><span className="font-mono text-[11px] text-slate-500">{product.ean || product.sku}</span></span><strong>×{product.displayedQuantity}</strong></div>)}</div>
    <div className="mt-4 rounded-md bg-slate-950 p-4 text-white">
      <div className="flex items-end justify-between"><span className="text-xs font-black uppercase text-slate-400">Ordini da prelevare</span><strong className="text-4xl font-black">{selected}<span className="text-lg text-slate-400">/{available}</span></strong></div>
      <Button className="mt-4 h-14 w-full bg-teal-600 text-base font-black text-white hover:bg-teal-500" onClick={() => onStart(group, selected)} disabled={working || selected < minimum}>{working ? <Loader2 className="mr-2 h-5 w-5 animate-spin" /> : <Play className="mr-2 h-5 w-5" />} Avvia con {selected} {selected === 1 ? "ordine" : "ordini"}</Button>
...
function MassMission({ batchId, mode }) {
  ...
  const currentStop = Math.max(1, routeStops.findIndex((line) => line.id === current?.id) + 1);
  const compactPicking = Boolean(current) && !complete && !bagConfirmation;
  return <div className={`wms-page pb-24 ${compactPicking ? "space-y-3" : ""}`} data-testid="wms-mass-picking-mission">
    {compactPicking ? <header className="flex items-center gap-3 rounded-md border border-slate-200 bg-white p-2.5 shadow-sm">
      <button type="button" onClick={() => navigate(`/wms-app/picking-${mode}`)} className="flex h-10 w-10 shrink-0 items-center justify-center rounded-md border border-slate-200 bg-white" aria-label="Torna alle missioni"><ArrowLeft className="h-5 w-5" /></button>
      <div className="min-w-0 flex-1">
        <div className="flex items-center justify-between gap-3"><span className="truncate text-[10px] font-black uppercase text-teal-700">Picking {mode === "mono" ? "mono-prodotto" : "massivo"} · tappa {currentStop}/{routeStops.length}</span><strong className="shrink-0 text-sm">{data.summary.picked}/{data.summary.expected}</strong></div>
        <Progress value={data.summary.progress} className="mt-2 h-1.5" />
      </div>
    </header> : <>
    ...
    {complete ? <section className="rounded-md border border-emerald-200 bg-emerald-50 p-6 text-center"><CheckCircle2 className="mx-auto h-12 w-12 text-emerald-700" /><h2 className="mt-4 text-2xl font-black">Bag registrata</h2><p className="mt-2 text-sm text-emerald-800">La bag {data.batch.bag_code} e stata consegnata al flusso packing. Da qui puoi solo consultarla nello storico.</p><Button className="mt-5 h-14 w-full text-base font-black" onClick={() => navigate("/wms-app/bag-storico")}><CheckCircle2 className="mr-2 h-5 w-5" /> Apri storico bag</Button></section> : bagConfirmation ? <section className="border-2 border-slate-950 bg-white p-5"><p className="text-xs font-black uppercase text-teal-700">Prelievo completato</p><h2 className="mt-1 text-2xl font-black">Scansiona una bag libera</h2><p className="mt-2 text-sm text-slate-500">Hai preso {data.summary.expected} pezzi. Mettili in una bag libera e scansionala ora: sarà occupata solo per questo packing.</p><Button type="button" className="mt-5 h-14 w-full text-base font-black" onClick={openScanner} disabled={working}><ScanLine className="mr-2 h-5 w-5" /> Scansiona bag</Button><form onSubmit={(event) => { event.preventDefault(); confirmBag(); }} className="mt-3 flex gap-2"><Input value={bagCode} onChange={(event) => setBagCode(event.target.value.toUpperCase().replace(/[^A-Z0-9-]/g, "").slice(0, 7))} placeholder="B-7K2Q9" className="h-14 flex-1 font-mono text-xl tracking-widest" /><Button type="submit" className="h-14 px-5" disabled={!bagPattern.test(bagCode) || working}><Barcode className="h-5 w-5" /></Button></form></section> : current && (needsSlot ? <section className="border-2 border-teal-500 bg-white p-3.5">
      <div className="flex items-center gap-3"><span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-md bg-teal-50 text-teal-800"><MapPin className="h-5 w-5" /></span><div className="min-w-0"><p className="text-[10px] font-black uppercase text-teal-700">Prossimo slot</p><h2 className="text-2xl font-black">{current.location?.codice}</h2></div></div>
      <div className="mt-3 flex items-center gap-3">{current.foto_url && <div className="flex h-24 w-28 shrink-0 items-center justify-center overflow-hidden rounded-md border border-slate-200 bg-white p-1.5"><img src={fileUrl(current.foto_url)} alt={current.titolo} className="h-full w-full object-contain" /></div>}<div className="min-w-0"><strong className="line-clamp-3 text-sm leading-5">{current.titolo}</strong><span className="mt-1 block truncate font-mono text-[10px] text-slate-500">{current.fnsku || current.ean || current.sku}</span></div></div>
      <Button className="mt-3 h-14 w-full text-base font-black" onClick={openScanner}><ScanLine className="mr-2 h-5 w-5" /> Scansiona slot</Button>
      <form onSubmit={(event) => { event.preventDefault(); scanSlot(); }} className="mt-2 flex gap-2"><Input value={code} onChange={(event) => setCode(event.target.value)} placeholder={current.location?.codice} className="h-11 flex-1 font-mono" /><Button type="submit" size="icon" variant="outline" className="h-11 w-11"><Barcode className="h-5 w-5" /></Button></form>
    </section> : <section className="border-2 border-teal-500 bg-white p-3.5">
      <p className="text-[10px] font-black uppercase text-teal-700">Preleva da {current.location?.codice}</p>
      <div className="mt-2 flex items-center gap-3">{current.foto_url && <div className="flex h-28 w-32 shrink-0 items-center justify-center overflow-hidden rounded-md border border-slate-200 bg-white p-1.5"><img src={fileUrl(current.foto_url)} alt={current.titolo} className="h-full w-full object-contain" /></div>}<div className="min-w-0"><h2 className="line-clamp-3 text-base font-black leading-5">{current.titolo}</h2><p className="mt-1 truncate font-mono text-[10px] text-slate-500">{current.fnsku || current.ean || current.sku}</p></div></div>
      <div className="mt-3 flex items-center justify-between rounded-md bg-slate-950 px-4 py-3 text-white"><span className="text-[10px] font-black uppercase text-slate-400">Selezionati</span><strong className="text-4xl font-black leading-none">{selectedQuantity}<span className="text-lg text-slate-400">/{remaining}</span></strong></div>
      <div className="mt-2 grid grid-cols-3 gap-2">{[1, 5, 10].map((amount) => <Button key={amount} variant="outline" className="h-14 text-xl font-black" onClick={() => addQuantity(amount)} disabled={selectedQuantity >= remaining}>+{amount}</Button>)}</div>
      <Button variant="ghost" className="mt-1 h-10 w-full" onClick={() => { selectedQuantityRef.current = 0; setSelectedQuantity(0); }} disabled={selectedQuantity === 0}>Azzera</Button>
    </section>)}
    {routeStops.length > 1 && <details className="rounded-md border border-slate-200 bg-white p-3"><summary className="cursor-pointer text-sm font-black">Percorso · {currentStop}/{routeStops.length}</summary><div className="mt-3 space-y-2">{routeStops.map((line, index) => { const done = Number(line.quantita_prelevata) >= Number(line.quantita_attesa); return <div key={line.id} className={`flex items-center gap-3 rounded-md border p-3 ${line.id === current?.id ? "border-teal-500 bg-teal-50" : done ? "border-emerald-200 bg-emerald-50" : "border-slate-200 bg-white"}`}><span className={`flex h-9 w-9 items-center justify-center rounded-full font-black ${done ? "bg-emerald-600 text-white" : "bg-slate-100"}`}>{done ? <CheckCircle2 className="h-5 w-5" /> : index + 1}</span><span className="min-w-0 flex-1"><strong className="block">{line.location?.codice} · {line.titolo}</strong><span className="text-xs text-slate-500">{line.quantita_prelevata}/{line.quantita_attesa} pezzi</span></span></div>; })}</div></details>}
