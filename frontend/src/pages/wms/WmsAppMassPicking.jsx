...
function MassQueue() {
  ...
  return <div className="space-y-5 pb-24" data-testid="wms-mass-picking-queue">
    ...
    {active.length > 0 && <section><h2 className="mb-3 text-xl font-black">Missioni aperte</h2><div className="space-y-3">{active.map((batch) => <button key={batch.id} type="button" onClick={() => navigate(batch.bag_code && ["completata", "in_packing"].includes(batch.stato) ? `/wms-app/packing/bag/${batch.bag_code}` : `/wms-app/picking-massivo/${batch.id}`)} className="flex w-full items-center gap-3 rounded-md border border-teal-200 bg-teal-50 p-4 text-left"><BagBadge code={batch.bag_code || "-"} /><span className="min-w-0 flex-1"><strong className="block">{batch.orders?.length || 0} ordini</strong><span className="mt-1 block text-xs text-teal-800">{statusLabel(batch.stato)}</span></span><ChevronRight className="h-5 w-5" /></button>)}</div></section>}
...
function MassMission({ batchId }) {
  ...
  const confirmBag = (rawCode) => { const value = String(rawCode || bagCode).trim().toUpperCase(); if (bagPattern.test(value)) { setBagCode(""); send({ codice: value }, "Bag confermata: vai alla packing station", "Bag non valida"); } };
  ...
  return <div className="space-y-5 pb-24" data-testid="wms-mass-picking-mission">
    ...
    <CameraScanner open={cameraOpen} onOpenChange={setCameraOpen} purpose={bagConfirmation ? "universal" : "location"} onDetected={(value) => { setCameraOpen(false); if (bagConfirmation) confirmBag(value); else scanSlot(value); }} />
  </div>;
}
