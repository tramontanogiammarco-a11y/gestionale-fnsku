...
function GalluseMission({ batchId }) {
  ...
  const scannerContext = awaitingCartScan
    ...
    : current
      ? { location: current.location?.codice, title: current.titolo, imageUrl: current.foto_url ? fileUrl(current.foto_url) : null, requested: remaining, locationConfirmed: !needsSlot, allowProductBarcode: Boolean(current.picking_scan_product_enabled), quantityControls: { value: quantity, remaining, working, onAdd: addQuantity }, bagAllocations, completedLines: (data.lines || []).filter((line) => Number(line.quantita_prelevata) >= Number(line.quantita_attesa)).length, totalLines: data.summary.stops, picked: data.summary.picked, expected: data.summary.expected }
      : null;
