...
function GalluseQueue() {
  ...
  return <div className="space-y-5 pb-24" data-testid="wms-galluse-queue">
    ...
    <section className="grid grid-cols-3 gap-2"><Metric label="Ordini sul carrello" value={round?.numero_ordini || 0} /><Metric label="Compiti" value="1" /><Metric label="Bag fisse" value="10" /></section>
    {active.length > 0 && <section><h2 className="mb-3 text-xl font-black">Carrello aperto</h2><button type="button" onClick={() => navigate(`/wms-app/picking-galluse/${active[0].id}`)} className="flex w-full items-center gap-3 rounded-md border border-teal-200 bg-teal-50 p-4 text-left"><span className="flex h-12 w-12 items-center justify-center rounded-md bg-white text-teal-700"><ShoppingBag className="h-6 w-6" /></span><span className="min-w-0 flex-1"><strong className="block">Carrello fisso da {active[0].orders?.length || active[0].numero_bag} ordini</strong><span className="mt-1 block text-xs text-teal-800">{active[0].stato === "da_associare_bag" ? "Associa le bag al carrello" : "Picking in corso"}</span></span><ChevronRight className="h-5 w-5" /></button></section>}
    <section><h2 className="mb-3 text-xl font-black">Compito disponibile</h2>{round ? <article className="rounded-md border border-slate-200 bg-white p-4 shadow-sm"><div className="flex items-start gap-3"><span className="flex h-12 w-12 items-center justify-center rounded-md bg-teal-50 text-teal-700"><Layers3 className="h-6 w-6" /></span><div className="min-w-0 flex-1"><h3 className="text-lg font-black">Compito 1 · {round.numero_ordini} ordini</h3><p className="mt-1 text-xs text-slate-500">{round.cliente} · {round.referenze} referenze · {round.pezzi} pezzi</p></div></div><p className="mt-4 rounded-md bg-slate-50 px-3 py-2 text-xs text-slate-600">Carrello fisso: bag B-73846 fino a B-73855 nelle posizioni 1-10. A ogni slot il sistema indica la distribuzione nelle bag.</p><Button className="mt-4 h-14 w-full text-base font-black" onClick={() => start(round)} disabled={working || active.length > 0}><Play className="mr-2 h-5 w-5" /> {active.length ? "Riprendi il carrello aperto" : "Avvia carrello"}</Button></article> : <Empty />}</section>
  </div>;
}
...
function GalluseMission({ batchId }) {
  ...
  return <div className="space-y-5 pb-24" data-testid="wms-galluse-mission">
    ...
    {!binding && !completed && <section className="rounded-md border border-slate-200 bg-white p-4"><p className="text-xs font-black uppercase text-slate-500">Carrello preconfigurato</p><p className="mt-1 text-sm text-slate-600">Le bag sono gia fisse nelle loro posizioni. Parti direttamente dal primo slot.</p><div className="mt-3 grid grid-cols-5 gap-2">{(data.orders || []).map((link) => <div key={link.id} className="rounded-md bg-slate-100 p-2 text-center"><strong className="block font-mono text-sm">{link.posizione_bag}</strong><span className="mt-1 block truncate font-mono text-[9px] text-slate-600">{link.bag_code}</span></div>)}</div></section>}
    {binding && <section className="rounded-md border border-teal-200 bg-teal-50 p-4"><p className="text-xs font-black uppercase text-teal-700">Fase 1 di 2</p><h2 className="mt-1 text-xl font-black text-teal-950">Associa prima le bag al carrello</h2><p className="mt-2 text-sm text-teal-900">Ora devi scansionare la bag per la posizione {nextBag?.posizione_bag}. Non vedrai ancora gli slot: compariranno automaticamente quando tutte le {data.summary.orders} bag saranno associate.</p><div className="mt-4 flex items-center justify-between rounded-md bg-white p-3"><span className="font-bold">Bag associate</span><strong className="font-mono text-xl">{data.summary.bags_ready}/{data.summary.orders}</strong></div><Button type="button" variant="outline" className="mt-3 h-12 w-full bg-white" onClick={openScanner} disabled={working}><ScanLine className="mr-2 h-5 w-5" /> Scansiona bag posizione {nextBag?.posizione_bag}</Button></section>}
