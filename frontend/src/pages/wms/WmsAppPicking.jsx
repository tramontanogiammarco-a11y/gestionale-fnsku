    ...
    <div className="wms-page pb-24" data-testid="wms-picking-mission">
      ...
      {!data.task ? (
        ...
        <section className="border-2 border-slate-950 bg-white p-5 shadow-sm">
          ...
          <form onSubmit={(event) => { event.preventDefault(); confirmBag(); }} className="mt-4 flex gap-2"><Input value={bagCode} onChange={(event) => setBagCode(event.target.value.toUpperCase().replace(/[^A-Z0-9-]/g, "").slice(0, 7))} placeholder="B-7K2Q9" className="h-14 flex-1 font-mono text-xl tracking-widest" autoFocus /><Button type="submit" className="h-14 px-5" disabled={!/^B-[A-Z0-9]{5}$/.test(bagCode) || working}><Barcode className="h-5 w-5" /></Button></form>
        </section>
      ) : (
        <>
