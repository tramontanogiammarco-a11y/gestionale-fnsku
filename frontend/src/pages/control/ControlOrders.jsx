...
export default function ControlOrders() {
  ...
  return <div><PageIntro eyebrow="Flusso outbound" title="Ordini" description="Segui ogni ordine dal suo ingresso fino alla spedizione. Le richieste di modifica vengono registrate come ticket tracciabili." action={<div className="flex flex-wrap gap-2"><Button variant="outline" onClick={recheckOrders} disabled={rechecking}>{rechecking?<Loader2 className="mr-2 h-4 w-4 animate-spin"/>:<RefreshCw className="mr-2 h-4 w-4"/>}Ricontrolla e sblocca</Button><Button variant="outline" onClick={()=>setCsvOpen(true)} disabled={context.isStaff && !context.clientId}><Upload className="mr-2 h-4 w-4"/>Importa CSV</Button><Button onClick={()=>setManualOpen(true)} disabled={context.isStaff && !context.clientId}><Plus className="mr-2 h-4 w-4"/>Nuovo ordine</Button></div>} />
    ...
    </Panel><ManualOrderDialog open={manualOpen} onOpenChange={setManualOpen} clientId={context.clientId} onCreated={loadOrders}/><CsvImportDialog open={csvOpen} onOpenChange={setCsvOpen} clientId={context.clientId} onImported={recheckOrders}/><OrderDetailDialog order={detailOrder} onOpenChange={(open)=>!open&&setDetailOrder(null)} onCarrier={()=>{setShippingOrder(detailOrder);setDetailOrder(null);}} onLoaded={loadOrders}/><ShippingQuoteDialog order={shippingOrder} onOpenChange={(open)=>!open&&setShippingOrder(null)} onSaved={loadOrders}/><EditOrderDialog order={editingOrder} onOpenChange={(open)=>!open&&setEditingOrder(null)} onSaved={recheckOrders}/><CancelOrderDialog order={cancellingOrder} onOpenChange={(open)=>!open&&setCancellingOrder(null)} onCancelled={loadOrders}/>
  </div>;
}
...
function ManualOrderDialog({ open, onOpenChange, clientId, onCreated }) {
  const empty = () => ({ order_number:"", ...EMPTY_ORDER_FORM, items:[{referenza_id:"",quantita:1}] });
  const [form,setForm]=useState(empty); const [references,setReferences]=useState([]); const [saving,setSaving]=useState(false);
  useEffect(()=>{
    if(!open)return;
    setForm(empty()); setReferences([]);
    const query=clientId?`?cliente_id=${encodeURIComponent(clientId)}`:"";
    api.get(`/referenze${query}`).then(({data})=>setReferences(data||[])).catch((error)=>toast.error(error.response?.data?.detail||error.message));
  },[open,clientId]);
  ...
  const save=async()=>{
    if(!form.order_number.trim())return toast.error("Inserisci il numero ordine");
    if(!form.ship_name.trim()||!form.ship_address1.trim()||!form.ship_zip.trim()||!form.ship_city.trim()||!form.ship_province.trim())return toast.error("Completa destinatario, indirizzo, CAP, città e provincia");
    if(!form.items.length||form.items.some((item)=>!item.referenza_id||!Number.isInteger(Number(item.quantita))||Number(item.quantita)<=0))return toast.error("Completa referenza e quantità di ogni prodotto");
    const referenceMap=new Map(references.map((reference)=>[reference.id,reference]));
    if(form.items.some((item)=>{const reference=referenceMap.get(item.referenza_id);return !reference?.ean&&!reference?.sku&&!reference?.fnsku;}))return toast.error("Una referenza selezionata non ha EAN, SKU o FNSKU");
    const rows=form.items.map((item)=>{const reference=referenceMap.get(item.referenza_id)||{};return {numero_ordine:form.order_number.trim(),ean:reference.ean||"",sku:reference.sku||"",fnsku:reference.fnsku||"",quantita:Number(item.quantita),titolo:reference.titolo||"",destinatario:form.ship_name,azienda:form.ship_company,indirizzo:form.ship_address1,indirizzo_2:form.ship_address2,cap:form.ship_zip,citta:form.ship_city,provincia:form.ship_province,paese:form.ship_country,codice_paese:form.ship_country_code,telefono:form.customer_phone,email:form.customer_email,note:form.note};});
    setSaving(true);
    try{const {data}=await api.post("/wms/ordini/import-csv",{cliente_id:clientId,rows,dry_run:false,source:"manual"});const result=data.gate_results?.[0];toast.success(result?.wms_status==="da_preparare"?`Ordine ${form.order_number} creato e pronto`:`Ordine ${form.order_number} creato e controllato`);close(false);await onCreated();}
    catch(error){toast.error(error.response?.data?.detail||error.message||"Ordine non creato");}
    finally{setSaving(false);}
  };
  return <Dialog open={open} onOpenChange={close}><DialogContent className="max-h-[92vh] overflow-y-auto sm:max-w-4xl"><DialogHeader><DialogTitle>Nuovo ordine manuale</DialogTitle><DialogDescription>Inserisci destinatario e prodotti. Al salvataggio partiranno automaticamente i controlli di indirizzo e stock.</DialogDescription></DialogHeader>
    <div className="space-y-6"><section><h3 className="mb-3 text-sm font-extrabold">Ordine</h3><OrderField label="Numero ordine" value={form.order_number} onChange={(value)=>field("order_number",value)}/></section>
      <section><h3 className="mb-3 text-sm font-extrabold">Destinatario e consegna</h3><div className="grid gap-3 sm:grid-cols-2"><OrderField label="Nome e cognome" value={form.ship_name} onChange={(value)=>field("ship_name",value)}/><OrderField label="Azienda" value={form.ship_company} onChange={(value)=>field("ship_company",value)}/><OrderField label="Indirizzo" value={form.ship_address1} onChange={(value)=>field("ship_address1",value)}/><OrderField label="Interno / scala" value={form.ship_address2} onChange={(value)=>field("ship_address2",value)}/><OrderField label="CAP" value={form.ship_zip} onChange={(value)=>field("ship_zip",value)}/><OrderField label="Città" value={form.ship_city} onChange={(value)=>field("ship_city",value)}/><OrderField label="Provincia" value={form.ship_province} onChange={(value)=>field("ship_province",value.toUpperCase())}/><OrderField label="Codice paese" value={form.ship_country_code} onChange={(value)=>field("ship_country_code",value.toUpperCase())}/><OrderField label="Telefono" value={form.customer_phone} onChange={(value)=>field("customer_phone",value)}/><OrderField label="Email" type="email" value={form.customer_email} onChange={(value)=>field("customer_email",value)}/></div></section>
      <section><div className="mb-3 flex items-center justify-between"><h3 className="text-sm font-extrabold">Prodotti nell’ordine</h3><Button type="button" variant="outline" size="sm" onClick={addItem}><Plus className="mr-2 h-4 w-4"/>Aggiungi prodotto</Button></div><div className="space-y-2">{form.items.map((item,index)=><div key={`${index}-${item.referenza_id}`} className="grid gap-2 rounded-md border border-slate-200 p-3 sm:grid-cols-[1fr_120px_40px] sm:items-end"><div><label className="text-xs font-bold">Referenza</label><Select value={item.referenza_id||undefined} onValueChange={(value)=>itemField(index,"referenza_id",value)}><SelectTrigger className="mt-1"><SelectValue placeholder="Seleziona prodotto"/></SelectTrigger><SelectContent>{references.map((reference)=><SelectItem key={reference.id} value={reference.id}>{reference.titolo} · {reference.sku||reference.ean||reference.fnsku}</SelectItem>)}</SelectContent></Select></div><div><label className="text-xs font-bold">Quantità</label><Input className="mt-1" type="number" min="1" step="1" value={item.quantita} onChange={(event)=>itemField(index,"quantita",Number(event.target.value))}/></div><Button type="button" variant="ghost" size="icon" className="text-rose-700" onClick={()=>removeItem(index)} disabled={form.items.length===1} aria-label="Rimuovi prodotto"><Trash2 className="h-4 w-4"/></Button></div>)}</div></section>
    ...
    </div><DialogFooter><Button variant="outline" onClick={()=>close(false)} disabled={saving}>Annulla</Button><Button onClick={save} disabled={saving||!references.length}>{saving?<Loader2 className="mr-2 h-4 w-4 animate-spin"/>:<Save className="mr-2 h-4 w-4"/>}Crea ordine</Button></DialogFooter>
  </DialogContent></Dialog>;
}
