...
export default function ControlOverview() {
  ...
  return <div>
    ...
    <div className="mt-4 grid gap-4 xl:grid-cols-2">
      ...
      <Panel title="Eccezioni recenti" description="Ordini e consegne che richiedono un intervento">{metrics.exceptions.length ? <div className="divide-y divide-slate-100">{metrics.exceptions.slice(0,6).map((row) => <div key={row.key} className="flex items-center gap-3 px-5 py-4"><AlertTriangle className="h-5 w-5 text-rose-600"/><div className="min-w-0 flex-1"><p className="truncate text-sm font-extrabold">{row.title}</p><p className="mt-1 text-xs text-slate-500">{formatDate(row.updated_at, true)}</p></div><StatusPill tone={row.kind === "stock" ? "amber" : "rose"}>{row.label}</StatusPill></div>)}</div> : <div className="flex min-h-72 items-center justify-center gap-3 text-sm font-bold text-emerald-700"><CheckCircle2 className="h-6 w-6"/> Nessuna eccezione attiva</div>}</Panel>
    </div>
  </div>;
}
...
function deriveMetrics(data) {
  ...
  const orderExceptions = data.orders.filter((order) => order.wms_status === "eccezione" || order.exception_type).map((order) => ({ key: `order-${order.id}`, kind: order.exception_type || "stock", title: order.order_name || "Ordine", label: order.exception_type === "indirizzo" ? "Indirizzo" : "Stock", updated_at: order.gate_checked_at || order.updated_at }));
  const shipmentExceptions = data.shipments.filter((shipment) => EXCEPTION_STATUSES.has(shipment.stato)).map((shipment) => ({ key: `shipment-${shipment.id}`, kind: "consegna", title: shipment.order?.order_name || shipment.tracking || "Spedizione", label: SHIPMENT_STATUS[shipment.stato] || shipment.stato, updated_at: shipment.tracking_updated_at || shipment.updated_at || shipment.created_at }));
