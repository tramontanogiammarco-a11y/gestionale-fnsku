    ...
    <div className="space-y-5" data-testid="admin-wms-control">
      <div className="flex flex-wrap items-end justify-between gap-4">
        ...
        <div className="flex gap-2">
          <Button variant="outline" onClick={() => navigate("/wms/mappa")}><MapPinned className="mr-2 h-4 w-4" /> Mappa 3D</Button>
...
function ControlView({ model, setView, navigate, onSelectOrder }) {
  const queue = [
    ...model.inbound.slice(0, 3).map((row) => ({ id: `in-${row.id}`, title: row.cliente_ragione_sociale, meta: `${row.tipo} · ${(row.righe || []).length} referenze`, label: row.stato === "in_lavorazione" ? "Continua" : "Ricevi", tone: "amber", action: () => navigate(`/wms/inbound/${row.id}`) })),
...
function PackingView({ rows, shipmentByOrder, creating, onSelect, onCreateShipment, navigate }) {
  return <div className="grid gap-4 lg:grid-cols-2">{rows.length === 0 ? <Card className="lg:col-span-2"><EmptyState icon={PackageCheck} text="Nessun ordine in packing" /></Card> : rows.map((order) => { const shipment = shipmentByOrder[order.id]; return <Card key={order.id} className="p-5"><div className="flex items-start justify-between gap-3"><div><button onClick={() => onSelect(order)} className="text-lg font-black hover:text-teal-700">{order.order_name}</button><div className="mt-1 text-xs text-muted-foreground">{order.cliente_ragione_sociale} · {orderPieces(order)} pezzi</div></div><OrderStatus value={order.wms_status} /></div><div className="mt-4 grid grid-cols-2 gap-2 rounded-md bg-slate-50 p-3 text-sm"><div><span className="text-xs text-muted-foreground">Righe</span><div className="font-bold">{order.items?.length || 0}</div></div><div><span className="text-xs text-muted-foreground">Outbound</span><div className="font-bold">{shipment ? shipment.stato : "Da creare"}</div></div></div><div className="mt-4 flex gap-2"><Button variant="outline" onClick={() => onSelect(order)}><ClipboardCheck className="mr-2 h-4 w-4" /> Distinta</Button>{shipment ? <Button onClick={() => navigate("/wms/ordini")}>Apri spedizione <ArrowRight className="ml-2 h-4 w-4" /></Button> : <Button disabled={creating === order.id} onClick={() => onCreateShipment(order)}>{creating === order.id ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Truck className="mr-2 h-4 w-4" />} Crea outbound</Button>}</div></Card>; })}</div>;
}
