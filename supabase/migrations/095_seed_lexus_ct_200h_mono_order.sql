-- Ordine Reliefbattery mono-prodotto per Batteria ibrida Lexus CT 200h 2010-2012.
do $$
declare
  v_client_id uuid;
...
begin
  ...
  where cliente_id = v_client_id
    and shop_domain = 'wms-mono-lexus-ct-200h-single.aimago.local';
  ...
  insert into public.shopify_orders (
    ...
    v_client_id, 'wms-mono-lexus-ct-200h-single.aimago.local',
    'RLB-MONO-LEXUS-CT-200H-20260903-001', '#RLB-MONO-LEXUS-CT-200H-001',
    'paid', null, 'da_preparare', 'sbloccato', now(), now(), now(), 0, 'EUR',
    'Cliente Test Lexus CT', 'Reliefbattery Test',
    'Via Lanciani 69', '00162', 'Roma', 'RM', 'Italia', 'IT',
    'mono.lexus.ct@example.com', '3333032001', 'gls', 4.00, 1, 'nazionale',
  ...
  insert into public.shopify_order_items (
    ...
    v_order_id, 'RLB-MONO-LEXUS-CT-200H-001-1', v_reference.id,
