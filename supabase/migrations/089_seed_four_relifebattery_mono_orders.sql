-- Quattro ordini Reliefbattery aggiuntivi per il picking mono-prodotto.
do $$
declare
  v_client_id uuid;
...
begin
  ...
  where cliente_id = v_client_id
    and shop_domain = 'wms-mono-relifebattery-extra.aimago.local';
  ...
  with products as (
    ...
    select available.*,
      row_number() over (
        order by md5(available.reference_id::text || 'RLB-MONO-EXTRA-4')
      ) as sequence
    from available
  ...
  if v_candidate_count <> 4 then
    raise exception 'Servono 4 SKU Reliefbattery libere con stock negli slot; trovate %', v_candidate_count;
  end if;
  ...
  for test_row in select * from tmp_mono_extra_selection order by sequence loop
    insert into public.shopify_orders (
      ...
      v_client_id,
      'wms-mono-relifebattery-extra.aimago.local',
      'RLB-MONO-EXTRA-20260903-' || lpad(test_row.sequence::text, 3, '0'),
      '#RLB-MONO-EXTRA-' || lpad(test_row.sequence::text, 3, '0'),
      ...
      'mono.extra.' || test_row.sequence || '@example.com', '3333020' || lpad(test_row.sequence::text, 3, '0'),
    ...
    insert into public.shopify_order_items (
      ...
      v_order_id,
      'RLB-MONO-EXTRA-' || lpad(test_row.sequence::text, 3, '0') || '-1',
  ...
  raise notice 'Creati 4 ordini mono-prodotto Reliefbattery aggiuntivi';
end;
$$;
