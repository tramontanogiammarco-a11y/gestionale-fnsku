-- Dieci ordini Reliefbattery da una sola unita, distribuiti su SKU e slot diversi.
do $$
declare
  v_client_id uuid;
...
begin
  ...
  where cliente_id = v_client_id
    and shop_domain = 'wms-mono-relifebattery-demo.aimago.local';

  create temporary table tmp_mono_selection on commit drop as
  with products as (
    select
    ...
    select available.*,
      ntile(10) over (
        order by coalesce(available.map_z, 0), coalesce(available.map_x, 0), available.location_code
      ) as route_bucket
    from available
    ...
    select spread.*,
      row_number() over (
        partition by spread.route_bucket
        order by md5(spread.reference_id::text || 'RLB-MONO-10')
      ) as bucket_rank
    from spread
  )
  select
    row_number() over (order by route_bucket)::integer as sequence,
    reference_id, titolo, ean, sku, fnsku, location_id, location_code, quantity
  from ranked
  where bucket_rank = 1
  order by route_bucket;

  select count(*) into v_candidate_count from tmp_mono_selection;
  if v_candidate_count <> 10 then
    raise exception 'Servono 10 SKU Reliefbattery libere con stock negli slot; trovate %', v_candidate_count;
  end if;

  for test_row in select * from tmp_mono_selection order by sequence loop
    insert into public.shopify_orders (
      ...
      v_client_id,
      'wms-mono-relifebattery-demo.aimago.local',
      'RLB-MONO-20260903-' || lpad(test_row.sequence::text, 3, '0'),
      '#RLB-MONO-' || lpad(test_row.sequence::text, 3, '0'),
      'paid', null, 'da_preparare', 'sbloccato',
      now(), now(), now() - make_interval(secs => 11 - test_row.sequence), 0, 'EUR',
      'Cliente Mono ' || lpad(test_row.sequence::text, 2, '0'), 'Reliefbattery Test',
      'Via Lanciani 69', '00162', 'Roma', 'RM', 'Italia', 'IT',
      'mono' || test_row.sequence || '@example.com', '3333010' || lpad(test_row.sequence::text, 3, '0'),
      'gls', 4.00, 1, 'nazionale',
      jsonb_build_object('carrier', 'gls', 'net', 4.00, 'source', 'mono_test'), now(),
      jsonb_build_object('valid', true, 'source', 'mono_test'),
      'Test picking mono-prodotto: SKU ' || test_row.sku || ' da ' || test_row.location_code,
      jsonb_build_object(
        'source', 'wms_mono_relifebattery_demo',
        'picking_mode', 'mono',
    ...
    insert into public.shopify_order_items (
      ...
      v_order_id,
      'RLB-MONO-' || lpad(test_row.sequence::text, 3, '0') || '-1',
      test_row.reference_id, test_row.sku, test_row.ean, test_row.titolo,
      1, 1,
      jsonb_build_object('source', 'wms_mono_relifebattery_demo', 'expected_location', test_row.location_code)
    );
    ...
    insert into public.wms_order_gate_events (
      ...
      jsonb_build_object('source', 'wms_mono_relifebattery_demo', 'stock_location', test_row.location_code)
    );
  end loop;

  raise notice 'Creati 10 ordini mono-prodotto Reliefbattery con 10 SKU distribuite negli slot';
end;
$$;
