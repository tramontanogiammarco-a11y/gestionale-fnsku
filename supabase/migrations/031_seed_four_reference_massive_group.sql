-- Six identical demo orders with four references for a separate Massivo picking group.
do $$
declare
  demo_client_id uuid;
  reference_ids uuid[];
...
begin
  ...
  if coalesce(array_length(reference_ids, 1), 0) < 10 then
    raise exception 'Servono almeno 10 referenze demo per il gruppo Massivo';
  end if;

  for order_index in 43..48 loop
    if exists (
    ...
    insert into public.shopify_orders (
      ...
      'WMS-MASS-' || lpad(order_index::text, 3, '0'),
      '#WMS-MASS-' || lpad(order_index::text, 3, '0'),
      'paid', null, 'da_preparare', now() - make_interval(mins => 49 - order_index),
      jsonb_build_object('source', 'wms_mass_demo', 'group', 'six-orders-four-references')
    ) returning id into demo_order_id;

    for line_index in 7..10 loop
