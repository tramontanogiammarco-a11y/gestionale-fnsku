-- Cinque ordini Reliefbattery richiesti per testare Galluse e picking massivo.
do $$
declare
  v_client_id uuid;
  v_request record;
  ...
  v_product_key text;
  v_order_id uuid;
  v_shop_domain constant text := 'wms-requested-bulk-20260907-112.aimago.local';
...
begin
  ...
  for v_request in
    select * from (values
      (1, 'SHOPIFY-57783761273219', 32, 'RLB-BULK32-112-001', '#RLB-BULK32-001'),
      (2, 'SHOPIFY-57783761568131', 3, 'RLB-MASS3-112-001', '#RLB-MASS3-001'),
      (3, 'SHOPIFY-57783761568131', 3, 'RLB-MASS3-112-002', '#RLB-MASS3-002'),
      (4, 'SHOPIFY-57783761568131', 3, 'RLB-MASS3-112-003', '#RLB-MASS3-003'),
      (5, 'SHOPIFY-57783761568131', 3, 'RLB-MASS3-112-004', '#RLB-MASS3-004')
  ...
  loop
    ...
    where reference.cliente_id = v_client_id
      ...
      and (
        upper(trim(coalesce(reference.ean, ''))) = upper(v_request.identifier)
        or upper(trim(coalesce(reference.sku, ''))) = upper(v_request.identifier)
        or upper(trim(coalesce(reference.fnsku, ''))) = upper(v_request.identifier)
      )
    order by reference.created_at
    limit 1;

    if not found then
      raise exception 'Referenza non trovata: %', v_request.identifier;
    end if;
    ...
    insert into public.shopify_orders (
      ...
      'Cliente Test ' || lpad(v_request.sequence::text, 2, '0'), 'Reliefbattery Test',
      'Via Lanciani 69', '00162', 'Roma', 'RM', 'Italia', 'IT',
      'bulk-test-' || v_request.sequence || '@example.com',
      '3332112' || lpad(v_request.sequence::text, 3, '0'), 'gls',
  ...
  if (select count(*) from public.shopify_orders where shop_domain = v_shop_domain) <> 5 then
    raise exception 'Creazione incompleta: attesi 5 ordini';
  end if;
