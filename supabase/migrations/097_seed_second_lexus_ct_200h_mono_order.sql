-- Ordine Reliefbattery mono-prodotto aggiuntivo per Lexus CT 200h 2010-2012.
do $$
declare
  v_client_id uuid;
...
begin
  ...
  where cliente_id = v_client_id
    and shop_domain = 'wms-mono-lexus-ct-200h-second.aimago.local';
  ...
  insert into public.shopify_orders (
    ...
    v_client_id, 'wms-mono-lexus-ct-200h-second.aimago.local',
    'RLB-MONO-LEXUS-CT-200H-20260904-002', '#RLB-MONO-LEXUS-CT-200H-002',
    'paid', null, 'da_preparare', 'sbloccato', now(), now(), now(), 0, 'EUR',
    'Cliente Test Lexus CT 02', 'Reliefbattery Test',
    'Via Lanciani 69', '00162', 'Roma', 'RM', 'Italia', 'IT',
    'mono.lexus.ct.02@example.com', '3333032002', 'gls', 4.00, 1, 'nazionale',
    ...
    'Ordine test aggiuntivo picking mono-prodotto Lexus CT 200h 2010-2012',
  ...
  insert into public.shopify_order_items (
    ...
    v_order_id, 'RLB-MONO-LEXUS-CT-200H-002-1', v_reference.id,
