-- Six identical demo orders, each with six slot-stocked references for Massivo picking.
do $$
declare
  demo_client_id uuid;
  reference_ids uuid[];
...
begin
  ...
  if coalesce(array_length(reference_ids, 1), 0) < 6 then
    raise exception 'Servono almeno 6 referenze demo per il gruppo Massivo';
  end if;

  for order_index in 37..42 loop
    if exists (
    ...
    insert into public.shopify_orders (
      ...
      '#WMS-MASS-' || lpad(order_index::text, 3, '0'),
      'paid', null, 'da_preparare', now() - make_interval(mins => 43 - order_index),
      jsonb_build_object('source', 'wms_mass_demo', 'group', 'six-orders-six-references')
    ) returning id into demo_order_id;

    for line_index in 1..6 loop
