...
Deno.serve(async (req) => {
  try {
    ...
    return json({ ok: true, authorize_url, redirect_uri: redirectUri, scopes: SCOPES, shop_domain: shopDomain });
  } catch (error) {
    console.error("shopify-oauth-start", error);
    return json({ detail: `Errore OAuth Shopify: ${error?.message || "errore interno"}` }, 500);
  }
});
